...
export default function HousingCard({ listing, onPress }: Props) {
  ...
  const members = listing.currentMembers ?? [];
  ...
  return (
    ...
    >
      <View style={styles.imageContainer}>
        ...
        />
        <View style={[styles.typeBadge, { backgroundColor: isOpenRoom ? colors.primary : isSublet ? '#7C3AED' : '#059669' }]}>
          <Text style={styles.typeBadgeText}>
            {isOpenRoom ? 'Open Room' : isSublet ? 'Sublet' : 'Forming Group'}
          </Text>
        </View>
        {isSublet && (
          <View style={styles.subletBadge}>
            <Feather name="clock" size={10} color="#7C3AED" />
            <Text style={styles.subletBadgeText}>Sublet</Text>
          </View>
        )}
      ...
      <View style={styles.body}>
        ...
        <View style={[styles.footer, { borderTopColor: colors.border }]}>
          {members.length > 0 ? (
            <>
              <MemberAvatars members={members} size={24} max={4} />
              <Text style={[styles.memberLabel, { color: colors.mutedForeground }]}>
...
const styles = StyleSheet.create({
  ...
  subletBadgeText: { color: '#7C3AED', fontSize: 10, fontWeight: '700' },
  rentBadge: {
    position: 'absolute',
    bottom: 10,
    right: 12,
