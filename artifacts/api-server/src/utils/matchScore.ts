 ...
 * Total = 100
 *
 * Additional bonus (outside 0–100 score):
 *   languageBonus       +3 max if ≥1 shared language
 */
...
function sleepScore(a: string | undefined, b: string | undefined): number {
...
function languageBonus(a: User, b: User): number {
  const aLangs = (a.language ?? "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
  const bLangs = (b.language ?? "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
  if (aLangs.length === 0 || bLangs.length === 0) return 0;
  const shared = aLangs.some((l) => bLangs.includes(l));
  return shared ? 3 : 0;
}
...
export function computeLanguageBonus(a: User, b: User): number {
  return languageBonus(a, b);
}

/**
 * Returns a per-category breakdown so clients can render "Why you match" details.
 * Religion is informational only — not factored into the numeric score.
 */
export function computeMatchBreakdown(
  ...
  const langBonus = languageBonus(a, b);

  const religionCompatible =
    !a.religion || !b.religion || a.religion === b.religion;
  ...
  if (langBonus > 0) {
    breakdown.push({ category: "Language", compatible: true, earned: langBonus, max: 3 });
  }

  if (a.religion || b.religion) {
    breakdown.push({
      category: "Religion (optional)",
      compatible: religionCompatible,
      earned: 0,
      max: 0,
    });
  }
