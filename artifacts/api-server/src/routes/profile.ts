...
import { UpsertProfileSchema, UserProfileSchema, PhotoUploadResponseSchema } from "@workspace/api-zod";
...
router.put("/profile/me", requireAuth, async (req, res) => {
  ...
  if (existing.length === 0) {
    ...
    // Allow privacy updates via profile/me
    if ((data as Record<string, unknown>).privacy !== undefined) {
      updateData.privacy = (data as Record<string, unknown>).privacy;
    }
  ...
  if (updated[0]) {
    ...
    updated[0].profileCompletion = completion;
  }
...
router.post("/profile/photo", requireAuth, upload.single("photo"), async (req, res) => {
  ...
  const key = `users/${req.userId}/${randomUUID()}.jpg`;
  ...
  if (bucketName) {
    try {
      ...
      // @ts-ignore — optional dependency; falls back to data URL when unavailable
      const { Client } = await import("@replit/object-storage");
      const client = new Client();
      await client.uploadFromBytes(key, processed, { contentType: "image/jpeg" });
      photoUrl = `https://storage.googleapis.com/${bucketName}/${key}`;
    } catch (err) {
      req.log.error({ err }, "Object storage upload failed, using fallback");
      // Fallback: store as data URL if object storage fails
      photoUrl = `data:image/jpeg;base64,${processed.toString("base64")}`;
    }
  } else {
    // No bucket configured — use data URL fallback
    photoUrl = `data:image/jpeg;base64,${processed.toString("base64")}`;
  }

  await db.update(usersTable).set({ photoUrl }).where(eq(usersTable.id, req.userId));
  ...
  sendValidated(res, PhotoUploadResponseSchema, { photoUrl });
});
